    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="app-navbar-collapse">
            <ul class="nav navbar-nav">
                ...
                <li class="{{ active_class(if_route('topics.index') || if_route('/')) }}"><a href="{{ route('topics.index') }}">话题</a></li>
                @foreach(Cache::get('larabbs_categories') as $key => $value)
                    <li class="{{ active_class(if_route('categories.show') && if_route_param('category',$key+1)) }}"><a href="{{route('categories.show',$key+1)}}">{{ $value->name }}</a></li>
                @endforeach
            </ul>
